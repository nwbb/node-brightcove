...
api.prototype.makeRequest = function makeRequest(command, params, callback, progressCallback) {
	...
	var formData = {};
	var fileData;
	var knownLength, stream; // Will only be defined on create_video & add_image calls
	...
	if(command === "create_video") {
		...
		fileData = handleFileAttachment(params.video.file, progressCallback);
		// Don't want this on the json data attr anymore as we have it as in a stream
		delete params.video.file;

	} else if(command === "add_image") {
		parseExtraAddImageParams(params);
		// https://support.brightcove.com/en/video-cloud/docs/adding-images-videos-media-api#remote

		if(params.image.file) {
      fileData = handleFileAttachment(params.image.file, progressCallback);
			// Don't want this on the json data attr anymore as we have it as in a stream
		 	delete params.image.file;
		}
	...
	if(fileData.stream) {
		...
		form.append('file', fileData.stream, fileData.knownLength !== undefined ? { knownLength: fileData.knownLength } : undefined);

		if (progressCallback) {
			var bytesSent = 0;
			var bytesTotal = form.getLengthSync();

			fileData.stream.on('data', function(chunk) {
...
// Could be expanded to pass back file checksum
function handleFileAttachment(file, progressCallback) {
...
function parseExtraAddImageParams(params) {
		// Move Video references off the image object
		if(params.image.video_id !== undefined) {
			params.video_id = params.image.video_id; 
			delete params.image.video_id;
		}
		if(params.image.video_reference_id !== undefined) {
			params.video_reference_id = params.image.video_reference_id; 
			delete params.image.video_reference_id;
		}
		if(params.image.checksum) {
			params.file_checksum = params.image.checksum;
			delete params.image.checksum; 
		}
}
...
function parseExtraCreateVideoParams(params) {
		...
		if(params.video.checksum) {
			params.file_checksum = params.video.checksum;
			delete params.video.checksum; 
		}
}
...
api.prototype.deleteVideoById = function deleteVideoById(videoId, options, callback) {
	...
	return this.makeRequest(this.commands.write.delete_video, opts, callback);
}
...
api.prototype.addThumbnailImage = function updateThumbnail(image, callback, progressCallback) {
	image.type = 'THUMBNAIL';
	return this.makeRequest(this.commands.write.add_image, { image: image }, callback, progressCallback);
}

api.prototype.addVideoStillImage = function updateVideoStill(image, callback, progressCallback) {

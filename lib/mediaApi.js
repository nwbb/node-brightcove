...
api.prototype.makeRequest = function makeRequest(command, options, callback, progressCallback) {
	...
	if(command === "create_video") {
		...
		if(isStream(params.video.file)) {
			if(progressCallback) {
				// Is there any clean way to get progress on streams without buffering?
				progressCallback = undefined;
				console.warn('progressCallback will not be called as streams are unknown in length')
			}

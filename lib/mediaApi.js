...
api.prototype.makeRequest = function makeRequest(command, options, callback) {
	...
	if(useWriteApi) {
		...
		if(command === "create_video") {
			...
			parseVideoParams(formData, params);
		}

		formData['JSONRPC'] = JSON.stringify({ method: command, params: params });
...
function parseVideoParams(form, params) {
		// Handle file uploading: Moves it from the params of the Video object we create, on to the form as a stream
		if (params.video.file) {
			var stream = isStream(params.video.file) ? params.video.file : fs.createReadStream(params.video.file);
			if(!isReadableStream(stream)) {
				throw new Error('Unable To Read From File Stream');
			}
			form.file = stream;
...
api.prototype.createVideo = function createVideo(video, callback) {
	...
	this.makeRequest(this.commands.write.create_video, { video: video }, callback);
}

api.prototype.updateVideo = function updateVideo(video, callback) {

	this.makeRequest(this.commands.write.update_video, { video: video }, callback);
}

api.prototype.deleteVideo = function deleteVideo(video, callback) {

	this.makeRequest(this.commands.write.delete_video, { video: video }, callback);
}
